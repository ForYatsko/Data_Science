...
# Убедитесь, что папки для сохранения графиков существуют
os.makedirs("output/visualizations", exist_ok=True)

def visualize_metrics(metrics, save_path="output/visualizations/metrics_comparison.png"):
    """
    Визуализирует метрики моделей в виде группированного столбчатого графика.
    Сохраняет график в указанный файл.
    """
    logging.info("Визуализация метрик моделей...")
    try:
        # Извлекаем модели и метрики
        model_names = sorted(set(key.split()[0] for key in metrics.keys()))  # Названия моделей
        metric_names = sorted(set(key.split()[1] for key in metrics.keys()))  # Названия метрик

        # Подготовка данных
        metric_values = {metric: [metrics.get(f"{model} {metric}", 0) for model in model_names] for metric in metric_names}

        # Построение графика
        x = np.arange(len(metric_names))  # Позиции групп (метрик)
        ...
        logging.info(f"График метрик сохранён в {save_path}.")
        ...
        logging.error(f"Ошибка при визуализации метрик: {e}")
        raise
